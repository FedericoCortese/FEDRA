...
# GAP and Silhouette for robust COSA ----------------------------------------------

compute_silhouette_grid <- function(X,
                                    zeta0,
                                    K,
                                    qt,
  ...
  dmat <- stats::dist(X, method = dist_method)
  ...
  worker <- function(i) {
    ...
    if (length(unique(cl)) < 2) {
      ...
      sil_obj <- cluster::silhouette(cl, dmat)
      sil_median <- median(sil_obj[, 3], na.rm = TRUE)
    }
  ...
  rownames(res) <- NULL
  res[order(-res$silhouette), ]
}


compute_gap_grid<- function(X,
                            zeta0,
                            qt,
                            K.max,
                            B = 20,
  ...
  worker <- function(i) {
    ...
    out <- tryCatch({
      ...
      data.frame(
        ...
        E.logW = gap_obj$Tab[, "E.logW"],
        gap = gap_obj$Tab[, "gap"],
        SE.sim = gap_obj$Tab[, "SE.sim"],
        K_opt_1se = K_opt,
        ok = TRUE
      )
    }, error = function(e) {
